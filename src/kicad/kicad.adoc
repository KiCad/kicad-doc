...
[width="100%",cols="20%,80%",]
|=================================================================
|*.sch |Schematic files, which does not contain the components themselves.
...
when using the KISYSMOD environment variable, is:
...
Select _Preferences/SetText Editor_ to set the text editor you want to use.
...
*WARNING: It is recommended to use a unique directory for each KiCad project.
...
(schemaic and PCB editors, gerber viewer and utility tools) when creating a design.
...
However, you can only edit the current project files.
...
|5 |*Gerbview* |A GERBER file viewer. It can also display drill files.
...
metadata. Templates facilitate the easy setup of projects which have
...
- When the environment variable KICAD_PTEMPLATES is defined there is a
  third page, Portable Templates, which lists templates found at the
