...
1.  Under Windows run kicad.exe. Under Linux type 'kicad' in your
    ...
    __PCB Calulator__ and __Pl Editor__. Refer to the work-flow chart
...
1.  Let us suppose that you have three 4-pin connectors that you want
    ...
    let's press the Ins key to have the same action automatically
    performed on the pin below pin 4 (PIN 3). Notice how the label is
...
+
NOTE: The 'Repeat last action' option accessible via the Ins key can
be successfully used to repeat period actions. For instance, the short
wires connected to all pins in Figure 2, Figure 3 and Figure 4 have
been placed with this option. Learn how to use it because it will make
using KiCad easier.
...
1.  From the KiCad project manager, click on the 'PCBNew' icon
    ...
    does not exist just ignore it and click Yes.
...
11. Now we will define the edge of the PCB. Select 'PCB Edges' from
...
12. Next, connect up all the wires except GND. In fact, we will
    ...
    on the bottom copper (called __Back__) of the board.
...
2.  Now we can start creating our new component. From KiCad, start
    ...
    some pins. Click on the 'Add Pins' icon image:images/pin.png[pin_png]
