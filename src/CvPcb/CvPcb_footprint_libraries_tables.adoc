...
Since December 2013, Pcbnew and CvPcb no longer uses the library management tool
...
footprint library table allows *direct use of footprint libraries* from
...
* you can write only KiCad  _.pretty_ footprint libraries file on your local disk.
...
Pcbnew also define this environment variable when loading a board file.
...
It can add "local" libraries or librairies from a Github repository.

When librairies are on a Github repository, they can be added as remote libraries,
or downloaded and added as local libraries.
...
Depending on the selected option, one of these page will be displayed,
