...
Modedit, which can help you to efficiently lay out components on the
canvas.
...
To use this tools, select the items you with to move and use either the hotkey
(defaults to Ctrl-M) or the context menu items to invoke the dialog. You
can also invoke the dialog with the hotkey which moving or duplicating
...
Pcbnew and the Module Editor both have assistants for creating arrays of
...
** *Continuous*: the numbering just continues across a row/column break - if
  ...
  row will be "8".  ** *Co-ordinate*: the numbering uses a two-axis scheme where
  the number is made up of the row and column index. Which one comes first (row
  or column) is determined by the numbering direction.
...
* *Angle*: The angular difference between two adjacent items in the
  array. Set this to zero to event divide the circle with "count" elements.
