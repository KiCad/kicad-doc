...
segments. As such each CAD application exports Ellipses as a series of small line
segments to 'approximate' an Ellipse. LibreCAD for example is known to do that.

Then also, CAD exports of Ellipses using such line segments tend not to 
'close' Ellipses. If the line segment lengths used to create the Ellipses are 
