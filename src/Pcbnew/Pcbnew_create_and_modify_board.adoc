...
As an alternative to drawing the board outline in Pcbnew directly, a 
board outline can also be imported from a DXF drawing. This can be used to 
create more complex board or allows board shapes to be defined in other 
CAD packages (e.g. to allow matching the board shape to a particular
...
segments to 'simulate' an Ellipses. LibreCAD for example is known to do that.

Then also, CAD exports of Ellipses using such line segments don't tend to 
'close' Ellipses, If the line segments used to create the Ellipses are too
big, this becomes visible as a gap in the board outline. 
