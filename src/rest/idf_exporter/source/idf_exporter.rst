...
Please direct any comments or suggestions to the author (cirilo_bernardo@yahoo.com).
...
The IDF exporter exports an IDFv3 [#f1]_ compliant board (\*.emn) and library
(\*.emp) file for communicating mechanical dimensions to a mechanical CAD
package. The exporter currently exports the board outline and cutouts,
all pad and mounting thru-holes including slotted holes, and component
outlines; this is the most basic set of mechanical data required for
interaction with mechanical designers. All other entities described in
...
and click on the 3D settings tab (see figure: :ref:`figure-1`), click on
Add 3D Shape, and select the filter “IDFv3 component files (\*.idf)” (see
figure: :ref:`figure-2`). Select the desired outline file and enter any
...
.. figure:: images/module_params.png
...
must be quoted. Quotation marks must not appear within a string. Floating
point numbers may be represented using decimal or exponential notations
but decimal notation is preferred for human readability. The decimal
point must be a dot and not a comma. The IDF file must consist only of
7-bit ASCII characters; use of 8-bit characters will result in undefined
behavior.
...
#.  Included Angle: a floating point number. If the value is 0 then a
    ...
    this point and if the value is positive then a counter-clockwise arc
    is drawn.
...
to give the user a general idea of what the outline is. For example axial
leaded cylindrical packages may represent some types of capacitors as
well as some types of resistors, so it makes sense to identify an outline
as a horizontal or vertical axial leaded device and to add some extra
information on the relevant dimensions: diameter, length, and pitch are
the most important. If a device has a unique outline, the manufacturer's
part number and a prefix to indicate the class of device are adequate.
...
MCAD system. The values of the strings will ideally have some meaning to
a user, but this is not necessary: the values are primarily intended for
the MCAD system to use as a unique ID. Ideally the values chosen will be
unique within any large collection of outlines; choosing values well will
result in fewer clashes especially in complex boards.
...
try to keep 2 in a horizontal arrangement on the X axis. Polarized
...
X axis and with the body of the device extending upwards (see figure:
:ref:`figure-4`). Non-polarized vertical axial leaded components must
...
.. figure:: images/test_idf_blobs.jpg

    Sample outlines

    Generated by the programs idfcyl and idfrect and rendered by
    SolidWorks.
...
example in a portable music player. In such a situation, if most extruded
outlines are a good enough representation of components then the
mechanical designer may only have to replace very few models while
...
#.  **idfcyl:** creates an outline of a cylinder in vertical or
    horizontal orientation and with axial or radial leads

#.  **idfrect:** creates an outline of a rectangle which may 
    have either an axial lead or a chamfer in the top left 
    corner

#.  **dxf2idf:** converts a drawing in DXF format into an IDF
    component outline
...
the free software `LibreCAD <http://librecad.org/>`_ for best
compatibility. When **dxf2idf** is invoked with no arguments it prints
out a usage note and a summary of its inputs:
...
Component file (*.emp) and produces a VRML file which can be viewed with
a VRML viewer. This feature is useful for visualization of the board
...
.. [#f1] http://www.simplifiedsolutionsinc.com/images/idf_v30_spec.pdf
