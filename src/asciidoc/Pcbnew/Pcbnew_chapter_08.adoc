...
Pcbnew allows you to define different routing parameters foe each
...
This dialog specify the smallest acceptable values for vias
...
routing can offer useful insight into the positioning of modules.
...
The button image:images/icons/net_highlight.png[] allows to highlight a
...
One can select a new size for routing, or to change a previously created
via or track segment:

image:images/Pcbnew_track_context_menu.png[]

If you want to change many vias (or tracks) size, the best way is to
use a specific Netclass to for the net(s) that must be edited (see
global changes).
