...
-   The net list file (\*.net) created by Eeschema with or without
    footprint associations.
-   The auxiliary component assignment file (\*.cmp) previously created
    by CvPcb if one exists.
...
to “Tools” menu or click on the generate net list button on the top tool
bar in Eeschema. By default the net list file has the same name as the
project with a “net” file extension. If the net list file for the
...
the “Open” entry in the “File” menu or clicking the “Open” file button
on the tool bar.
...
![](images/1000000000000324000001ED7EE8E1AB.png)
...
loaded and the footprint pane can be also empty if no footprint
libraries are found.
...
------------------------------------------------ -------------------------------------------------
![](images/icon_open_document.png)               Select the net list file to be processed.

![](images/icon_save.png)                        Save the footprint association file (.cmp) and
                                                 the updated net list (.net) file.

![](images/icon_config.png)                      Invoke the CvPcb configuration menu.

![](images/icon_show_footprint.png)              Display the footprint of the component selected
                                                 in the footprint window.

![](images/100002010000001A0000001AFDF35759.png) Automatically associate footprints with
                                                 components starting using an equivalence file.

![](images/100002010000001A0000001A3EEC8BF8.png) Automatically select the previous component in
                                                 the list without a footprint association.

![](images/100002010000001A0000001A35181776.png) Automatically select the next component in the
                                                 list without a footprint association.

![](images/icon_delete_association.png)          Delete all footprint assignments.

![](images/icon_datasheet.png)                   Open the selected footprint documentation pdf
                                                 file using the default pdf viewer.

![](images/100002010000001A0000001A2030B04B.png) Enable or disable the filtering to limit the list
                                                 of footprints to the footprint filters of the
                                                 selected component.

![](images/100002010000001A0000001A98870FF1.png) Enable or d isable the filtering to limit the
                                                 list of footprints using the pin count of t he
                                                 selected component.

![](images/100002010000001A0000001A7D355C45.png) Enable or disable filtering to limit the list of
                                                 footprints using the selected library.
------------------------------------------------ -------------------------------------------------
...
The following table lists the keyboard commands for the main window:
  ...
  ----------- ------------------------------------------------------------
  Right Arrow Activate the next pane to the right of the currently
  / Tab       activated pane. Wrap around to the first pane if the last
              pane is currently activated.

  Left Arrow  Activate the next pane to the left of the currently
              activated pane. Wrap around to the last pane if the first
              pane is currently activated.
...
![](images/10000000000000D3000000C24C09C33B.png)
...
-   The legacy management, using \*.mod files, and a library list of
    files
...
![](images/2000000900003A6600000EC90C50E7EF.png)
...
![](images/2000000900003A1700000596F88F93A0.png)
...
![](images/1000000000000230000000DF5519F9DB.png)
...
remove the selected user defined search path. Clicking the “Remove”
button will have no effect if a default search path is selected.

![](images/100000000000022F0000007155E2D729.png)

### Default Library Paths

By default CvPcb internally uses a set of predefined paths used to
search for footprint libraries. These paths are operating system
dependent. It is generally preferable to use relative paths rather than
absolute paths whenever possible to prevent platform dependency
problems. In other words: “c:Program Fileskicadshare” has no meaning and
will fail on Linux and OSX.

The default Linux library paths will be as follows:

-   **root/share/kicad/modules**
-   **root/share/kicad/modules/packages3d** (for 3D shapes files format
    **VRML** created par Wings3D).
-   **root/share/template**
...
5.1](#5.1.Legacy%20management:%20selecting%20Footprint%20Libraries|outline).
...
![](images/10000000000003E40000028C68557401.png)
...
-   Please note that you cannot have duplicate library nicknames in the
    same table. However, you can have duplicate library nicknames in
    both the global and project specific footprint library table.
-   The project specific table entry will take precedence over the
    global table entry when duplicated names occur. When entries are
    defined in the project specific table, an fp-lib-table file
    containing the entries will be written into the folder of the
    currently open net list.
...
provides "Copy On Write" (COW) support f or edit ing footprints read
from the GitHub repo and saving them locally. Therefore the "GitHub"
plugin is for **read only for accessing remote pretty footprint
libraries at [<https://github.com>](https://github.com/).** To add a
GitHub entry to the footprint library table the "Library Path" in the
...
path are com bined with the read only part of the GitHub repository to
create the footprint library. If th is option is missing, then the Git H
ub library is read only. If the option is present for a Git H ub
library, then any writes to this hybrid library will go to the
local \*.pretty directory. Note that the github.com resident portion of
...
**allow\_pretty\_writing\_to\_this\_dir**. Once you have saved a
footprint to the COW library's local directory by doing a footprint save
in the footprint editor, no GitHub updates will be seen when loading a
...
Simply email the individual \*.kicad\_mod files you find in your COW
...
![](images/2000000900000C4E00001A91ED722582.png)
...
------------------------------------------------ -----------------------------------------------------------------------------------------------------------------------------------------------
![](images/100002010000001A0000001AFFE7B356.png) Show display options dialog

![](images/100002010000001A0000001A987F0D04.png) Zoom in

![](images/100002010000001A0000001A66ABF68B.png) Zoom out

![](images/100002010000001A0000001A798AA253.png) Redraw

![](images/100002010000001A0000001A0D57E008.png) Fit drawing in display area

![](images/100002010000001A0000001A2C2F6349.png) Open 3D model viewer
------------------------------------------------ -----------------------------------------------------------------------------------------------------------------------------------------------

### Vertical Toolbar

------------------------------------------------ -----------------------------------------------------------------------------------------------------------------------------------------------
![](images/100002010000001A0000001A1103DCA9.png) Show or hide the grid

![](images/100002010000001A0000001A3CD4BE9F.png) Show coordinates in polar or rectangular notation

![](images/100002010000001A0000001A59E37A09.png) Display coordinates in inches

![](images/100002010000001A0000001AD542C4CF.png) Display coordinates in millimeters

![](images/100002010000001A0000001A4A78FB18.png) Toggle cursor style

![](images/100002010000001A0000001A68CAF66E.png) Toggle between drawing pads in sketch or normal mode

![](images/100002010000001A0000001A406F2807.png) Toggle between drawing text in sketch or normal mode

![](images/100002010000001A0000001A09E18AFA.png) Toggle between drawing edges in sketch or normal mode
------------------------------------------------ -----------------------------------------------------------------------------------------------------------------------------------------------
...
![](images/10000000000002C7000002264738B528.png)
...
------------------------------------------------ ----------------------------------------------
![](images/100002010000001A0000001A106F8A4B.png) Reload the 3D model

![](images/100002010000001A0000001A9A14D17F.png) Copy 3D image to clipboard
...
![](images/100002010000001A0000001A1DC691C6.png) Rotate backward along the X axis

![](images/100002010000001A0000001A36E79F92.png) Rotate forward along the X axis

![](images/100002010000001A0000001A1C53FE12.png) Rotate backward along the Y axis

![](images/100002010000001A0000001A94DDCA95.png) Rotate forward along the Y axis

![](images/100002010000001A0000001A9C45D193.png) Rotate backward along the Z axis

![](images/100002010000001A0000001AF4723C21.png) Rotate forward along the Z axis

![](images/100002010000001A0000001A3EEC8BF8.png) Pan left

![](images/100002010000001A0000001A35181776.png) Pan right

![](images/100002010000001A0000001AB8AAD9D8.png) Pan up

![](images/100002010000001A0000001A38DC77BD.png) Pan down

![](images/100002010000001A0000001AAC4BB5EB.png) Toggle orthographic projection mode on and off
------------------------------------------------ ----------------------------------------------
...
![](images/10000000000003240000021792B3B03C.png)
...
![](images/1000000000000324000002177D2E4BF4.png)
...
![](images/10000000000001AE0000013893C2B4F6.png)

Filtered by the pin count of the selected component.

![](images/10000000000003240000021766744A5C.png)

Filtered by the selected library.

![](images/100000000000032400000217206EB343.png)

The icons
![](images/10000000000000400000001FB5B88EB2.png)
...
![](images/100000000000032400000217E1D94A37.png)
