...
hotkeys. Both are used to speed up working in KiCad by avoiding moving the mouse.

==== Accelerator keys
Accelerator keys have the same effect as clicking on a menu or toolbar icon:
the mode will be entered but nothing will happen until the left mouse button
is clicked. Use an accelerator key when you want to enter a mode but do not
want any immediate action.

Accelerator keys are shown on the right side of all menu panes:
image::images/gsik_accelerator_keys.png[gsik_accelerator_keys]

=== Hotkeys
A hotkey is equal to an accelerator key plus a left mouse click. Using a
hotkey starts the command immediately at the current cursor location where
the key is typed. Use a hotkey to quickly change modes without interrupting
your workflow.

To view hotkeys within any KiCad tool go to *_Preferences -> Hotkeys ->
List Current Keys_* or press the question mark ("?"):
image::images/gsik_hotkeys.png[gsik_hot_keys]

You can edit the assignment of hotkeys, and import or export them, from the
*_Preferences -> Hotkeys_* menu.
...
To use an accelerator key, press "Shift + W" to enter the "Add wire" mode (note
the cursor will change). Next, left click on the desired wire start location
to begin drawing the wire.
...
18. Change the grid size. You have probably noticed that on the
    ...
    select**. __In general, it is recommended to use a grid of 50.0 mils
...
8.  If you want to fill the rectangle with yellow, set the fill colour
    ...
    the rectangle in the editing screen and edit (E), selecting
    'Fill background'.
