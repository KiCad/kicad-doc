...
|Pcbnew |Circuit board board and footprint editor|+*.kicad_pcb, *.kicad_mod+
|GerbView |Gerber and drill file viewer|All the usual Gerbers, +*.drl+
...
5.  We will now place our first component. Click on the 'Place
    ...
    in the right toolbar. You may also press the 'Place component' hotkey [a].
...
+
NOTE: *Right-Click* -> *Move component* or [m] is also a valuable option
...
xsltproc.exe is located at _<KiCad install directory>\bin_, while the included
*.xsl files can be found at _<KiCad install directory>\bin\scripting\plugins_.

*Linux:*

The *.xsl files are located in the _plugins_ directory of the KiCad installation,
which is located at: _/usr/lib/kicad/plugins/_.

You can get bom2csv.xsl via:
---------
wget https://raw.githubusercontent.com/KiCad/kicad-source-mirror/master/eeschema/plugins/bom2csv.xsl
---------

If xsltproc is missing, you can downloaded it with:
---------
