...
For example, you could create an alias called "USB" that refers to the nets "DP" and "DM"
Then, you can define a bus group as "{USB}" which will contain both of those nets.
You can also add a label, like "USB1{USB}" which will result in nets "USB1.DP" and "USB1.DM"
Bus Aliases are editable through a new dialog, and saved with the schematic file.
